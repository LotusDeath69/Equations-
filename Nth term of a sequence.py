...
def Quadratic(x1, y1, x2, y2, x3, y3):
    ...
    if Verify:
        if Formatting:
            '#Reitirating a, b, c values because some of them might have been converted into a str'
            d = Symbol('d')
            e = Symbol('e')
            equation = a * x1 ** 2 + d * x1 + e - y1
            equation_2 = a * x2 ** 2 + d * x2 + e - y2
            [d, e] = nsolve([equation, equation_2], [d, e], [1, 1])
            a = second_difference/2
            if a * x3 ** 2 + d * x3 + e == y3:
                print('Verified')
        else:
